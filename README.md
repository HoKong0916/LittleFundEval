...
**为什么这样选**：成本结构。路由分类、话题判断、REWOO 阶段1 提取基金名——这些是**高频低智任务**，每轮对话都要跑，用云端大模型烧钱且没必要。本地 Qwen2.5 9B 量化版跑这些任务几乎零边际成本，DeepSeek 只为真正的推理（ReAct 多步、REWOO 综合）付费。
...
### 预算管控：为什么是软限 + 阈值，而非预扣对账

**真问题**：云端 LLM 按量计费，必须限预算。最初考虑过"预扣输入 + max_tokens 占坑，事后用真实 usage 补差退回"的精确计费方案，最终没采用。

**为什么不用预扣**：
1. ReAct 一次 run_chat 内最多 5 步 cloud_chat，每步都要预扣+补差，异常路径（CancelledError/超时）的退回逻辑分支多，复杂度高
2. 飞书侧有会话锁（同 open_id 同时只跑一个 run_chat），单用户并发超扣不存在
3. API 侧定位为调试，并发不是真威胁

**实际采用的方案**：
- **TTL 到当天 23:59:59**：避免 `ex=43200(12h) + nx=True` 组合导致的"当天额度翻倍" bug（早 8 点用，晚 8 点 key 过期后 nx=True 重新初始化满额）
- **check 阈值 `remaining > 2000`** 而非 `> 0`：防"剩余 1 token 照发 5 万"——ReAct 单次调用 3-5k token，剩 1 也放行就会单请求超额
- **DECRBY 扣真实 total_tokens**：DeepSeek 返回的权威值，事后扣减

这套方案在中小企业单租户场景下够用，面试讲预算管控时能讲清"为什么不做预扣"的取舍。

### 降级熔断：为什么不是"降级即放行"

**原设计的问题**：`local_chat` 连不上 llama-server 时静默切到 DeepSeek，路由/摘要/REWOO 提取三个高频本地调用全在无感烧云端钱，且不扣预算。llama-server 挂半小时烧的钱可能比正常态一天还多。

**为什么不在降级路径精确扣费**：降级是异常态，不是常态。在异常态做精确计费是过度工程——目标是"快速止血"而非"精确记账"。
...
- **大模型**：本地 `llama.cpp`（Qwen2.5 9B GGUF，路由分类）+ DeepSeek 云端（流式推理 + Function Calling），互为降级
...
# llama-server -m models/Qwen2.5-9B-Instruct-Q4_K_M.gguf --port 9856
```
