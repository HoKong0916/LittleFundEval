"""router.py 专用 prompt —— 意图分类器，将用户问题路由到 DirectAnswer / ReAct / REWOO 策略。

与 core/router.py 配合：LLM 分析用户问题 + 可用工具列表 + 历史上下文，决定使用哪种执行策略及需要哪些工具。
"""
...
1. **历史数据完全够用** → DirectAnswer
   历史对话中已有具体数据（资金流向数值、基金收益率、持仓等），且用户当前问题**仅需基于这些已有数据给出分析、建议或意见**。无需重复查询，tools_needed 为空。

2. **历史有部分数据，但引入了新基金或新维度** → ReAct 或 REWOO
   历史已包含某些基金的数据，但用户引入了**历史中没有的新基金**或**新的查询维度**，仍需调工具——但只查缺失部分。**tools_needed 只列出缺失部分所需的工具**，不要重复列出历史已有数据对应的工具。
   例：历史已有中海科技创新混合C的收益持仓，用户追问"德邦鑫星灵活价值混合C也是光模块的，跟它比谁更好？"→ 中海数据已有，tools_needed 只需列出德邦鑫星相关的工具（如 search_fund、get_fund_performance、get_fund_holdings）。

3. **历史中完全没有相关数据** → ReAct 或 REWOO（按下方规则正常判断）

关键判断：逐只基金、逐维度检查，**只对历史中没有的部分调工具**。
...
问题需要对一个已明确指定的基金（或几只明确基金）进行多维度综合分析。大部分数据工具（get_fund_performance、get_fund_holdings 等）之间没有查询依赖，可以并发获取。

典型标志：问题中直接给出了基金（名称或代码均可），且要求"全面分析"、"评估一下"、"各方面怎么样"、"对比一下"。

例："全面分析一下 519702，各方面都想了解一下。"

例："对比一下 005827 和 260108 的收益、风险和持仓。"

重要规则：
- 即使问题中提到了具体基金，如果只涉及单一维度查询，应走 ReAct 而非 REWOO。
- **名称→代码依赖**：当基金以中文名称（非6位纯数字代码）给出时，search_fund 是数据工具的前置依赖。此时仍需在 tools_needed 中列出 search_fund 和所有数据工具——执行引擎会自动处理为"先 search_fund 解析代码，再并发执行数据工具"。
- tools_needed 必须列出所有需要调用的工具（包括 search_fund），不要省略。
- **上下文感知**：如果历史对话中已包含某些基金的数据，tools_needed 只列出新基金的查询工具，不要重复查询历史已有数据。
...
**示例 12：REWOO（多只基金对比，名称需搜代码）**
用户：对比一下易方达蓝筹精选和景顺长城新兴成长的收益、风险和持仓。
输出：
{
  "category": "REWOO",
  "tools_needed": ["search_fund", "get_fund_performance", "get_fund_holdings"],
  "reasoning": "两只基金给的是名称，执行引擎先搜代码再并发拉数据"
}

**示例 13：DirectAnswer（历史已有数据，用户仅追问建议）**
历史消息：助手已回复某几个板块今日资金流向数据。
用户：（基于上述数据追问）那我持有这个板块，需要减仓吗？后续该往哪个方向调仓？
输出：
{
  "category": "DirectAnswer",
  "tools_needed": [],
...
**示例 15：REWOO（多只基金对比，代码明确，无需search_fund）**
用户：对比一下 005827 和 260108 的收益、风险和持仓。
输出：
{
  "category": "REWOO",
  "tools_needed": ["get_fund_performance", "get_fund_holdings"],
  "reasoning": "两只基金代码明确，多维度无依赖，可并发查询"
}

**示例 16：REWOO（部分历史已有，仅查新基金+名称需搜代码）**
历史消息：助手已回复中海科技创新混合C的收益、持仓数据。
用户：据我了解德邦鑫星灵活价值混合C也是光模块的，它和中海科技创新混合C哪个更优？
输出：
{
  "category": "REWOO",
  "tools_needed": ["search_fund", "get_fund_performance", "get_fund_holdings"],
  "reasoning": "中海数据历史已有，仅查德邦鑫星（名称先搜代码，再并发拉数据）"
}
